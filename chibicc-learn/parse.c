...
static Type *type_suffix(Token **rest, Token *tok, Type *ty) {
  if(equal(tok, "(")) {
    ...
    while(!equal(tok, ")")) {
      ...
      // TODO: ここでコピーするのは、ty はポインタで、次のループで上書きされる可能性がある？いや、ループのたびに定義されるからそうはならないのでは？なぜわざわざ新しいメモリを確保しているのか？
...
static Type *declarator(Token **rest, Token *tok, Type *ty) {
  ...
  // なぜtype_suffixを呼ぶのか謎だったが、関数定義かどうかを判別できるポイントはここだから、だった
...
// TODO:
// 自作のポインタ演算では良い感じに実装できなかったところをスマートに書いている。とても参考になる
